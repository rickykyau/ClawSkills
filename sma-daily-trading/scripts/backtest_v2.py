#!/usr/bin/env python3
"""
SMA Daily Trading Strategy Backtest v2
- 15% TRAILING STOP ONLY (no fixed stop)
...
def run_backtest(data_start='2021-01-01', backtest_start='2021-07-01', backtest_end='2026-02-05', 
                 capital=10000, sma_period=100, trailing_stop_pct=15, tax_rate=30):
    
    print("═"*100)
    print(" STRATEGY: QQQ Signal → TQQQ Trade (Trailing Stop Only)")
    print("═"*100)
    print()
    print("| Rule           | Value                                                        |")
    print("|----------------|--------------------------------------------------------------|")
    ...
    print("| SELL           | QQQ crosses below 100 SMA OR trailing stop hit               |")
    ...
    for i in range(1, len(qqq)):
        ...
        if position:
            position['high_since_entry'] = max(position['high_since_entry'], tqqq_high)
            stop_price = position['high_since_entry'] * (1 - trailing_stop_pct/100)
    ...
    print("─"*100)
    print(f"{'#':<4} {'Entry':<12} {'Exit':<12} {'Entry$':>8} {'Exit$':>8} {'P&L%':>8} {'P&L$':>10} {'Days':>5} {'Exit':>6} {'Entry':>6}")
    print("─"*100)
    
    for i, t in enumerate(trades):
        entry_str = t['entry_date'].strftime('%Y-%m-%d')
        exit_str = t['exit_date'].strftime('%Y-%m-%d')
        print(f"{i+1:<4} {entry_str:<12} {exit_str:<12} ${t['entry_price']:>6.2f} ${t['exit_price']:>6.2f} {t['pnl_pct']:>+7.1f}% ${t['pnl']:>+8.0f} {t['days']:>5} {t['exit_reason']:>6} {t['entry_reason']:>6}")
    ...
    print(f"Trailing Stop Exits:{stop_exits}")
...
def main():
    parser = argparse.ArgumentParser(description='SMA Daily Trading Backtest v2 (Trailing Stop Only)')
    ...
    parser.add_argument('--trailing-stop', type=float, default=15, help='Trailing stop %')
