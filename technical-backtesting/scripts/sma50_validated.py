#!/usr/bin/env python3
"""
SMA(50) Crossover - V8: V3 + EOD exit + no T+1 after EOD exits.
Based on V3 (32 matches) + V6 trade 33 fix + T+1 bypass for EOD.
"""
import os
import pandas as pd
import numpy as np
...
for ts in common_ts:
    ...
    if pending_eod_exit and is_first_bar and in_position:
        ...
        shares = 0
        last_exit_date = d
        last_exit_was_eod = True  # Don't enforce T+1
        pending_eod_exit = False
        # Set prev_above_sma to False (daily close was below SMA)
        prev_above_sma = False
        # Don't skip this bar for potential re-entry
    ...
    if in_position:
        ...
        if tqqq_low <= active_stop:
            exit_reason = "STOP"
            # Gap-through slippage: if bar opened below stop, fill at close
            if tqqq_high < active_stop:
                exit_price = tqqq_close  # Gap through - can't fill at stop
            else:
                exit_price = active_stop  # Normal stop fill
        elif prev_above_sma and not above_sma and not is_last_bar:
            # SMA exit - but NOT on last bar of day (let EOD check handle it)
    ...
    # CHECK ENTRIES
        ...
        # T+1 cooldown: skip if exited today UNLESS it was an EOD exit
        ...
        in_position = True
        last_exit_was_eod = False  # Reset after re-entry
    ...
    if ts == last_bar_of_day.get(d) and d in sma_by_date and d in daily_close_by_date:
        ...
        # Reset prev_above_sma to EOD state
