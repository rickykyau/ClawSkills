#!/usr/bin/env python3
"""
SMA(50) Crossover - V7: V3 base + key QC behaviors:
1. Last bar of day uses TODAY's SMA (QC daily consolidator updates at close)
2. After last bar, reset prev_above_sma to daily close vs daily SMA state
3. No T+1 cooldown after EOD-triggered exits
"""
import os
import pandas as pd
import numpy as np
...
for ts in common_ts:
    ...
    # Key QC behavior: last bar uses today's SMA (daily consolidator updates at close)
    if is_last_bar and d in sma_by_date:
        sma_val = sma_by_date[d]
    else:
        sma_val = prev_sma_by_date[d]
    ...
    # At end of day, reset prev_above_sma to daily close vs daily SMA
    if is_last_bar and d in sma_by_date and d in daily_close_by_date:
        prev_above_sma = daily_close_by_date[d] > sma_by_date[d]
