#!/usr/bin/env python3
"""
Massive Grid Search v2 - 1000+ Strategy Combinations
...
def main():
    ...
    print(f"Strategy parameter combinations: {len(param_combos)}")
    print(f"Symbols: {len(SYMBOLS)}")
    print(f"Timeframes: {len(TIMEFRAMES)}")
    print(f"Total combinations: {len(param_combos) * len(SYMBOLS) * len(TIMEFRAMES)}")
    print()
    ...
    print(f"\nFetched data for {len(data_cache)} symbol/timeframe combinations")
    print()
    ...
    for key, df in data_cache.items():
        ...
        print(f"  {symbol} {tf}: {len([r for r in results if r['params']['symbol']==symbol and r['params']['timeframe']==tf])} valid strategies | Progress: {pct:.0f}%")
    ...
    print("=" * 80)
    print("TOP 25 STRATEGIES BY TOTAL RETURN")
    print("=" * 80)
    ...
    for i, r in enumerate(by_return, 1):
        ...
        print(f"   RSI({p['rsi_period']}) OS:{p['rsi_oversold']} OB:{p['rsi_overbought']} | MACD({p['macd_fast']},{p['macd_slow']},{p['macd_signal']})")
        print(f"   SL:{p['stop_loss_pct']}% TP:{p['take_profit_pct']}%")
        print(f"   Return: {r['total_return']:+.1f}% | Trades: {r['num_trades']} | Win Rate: {r['win_rate']:.0f}% | PF: {r['profit_factor']} | Max DD: {r['max_drawdown']:.1f}%")
    
    print()
    print("=" * 80)
    print("TOP 25 BY PROFIT FACTOR (min 10 trades)")
    print("=" * 80)
    ...
    for i, r in enumerate(by_pf, 1):
        ...
        print(f"   RSI({p['rsi_period']}) OS:{p['rsi_oversold']} OB:{p['rsi_overbought']} | MACD({p['macd_fast']},{p['macd_slow']},{p['macd_signal']})")
        print(f"   SL:{p['stop_loss_pct']}% TP:{p['take_profit_pct']}%")
        print(f"   PF: {r['profit_factor']} | Return: {r['total_return']:+.1f}% | Trades: {r['num_trades']} | Win Rate: {r['win_rate']:.0f}% | Max DD: {r['max_drawdown']:.1f}%")
    
    print()
    print("=" * 80)
    print("TOP 25 BY WIN RATE (min 10 trades)")
    print("=" * 80)
    ...
    for i, r in enumerate(by_wr, 1):
        ...
        print(f"   RSI({p['rsi_period']}) OS:{p['rsi_oversold']} OB:{p['rsi_overbought']} | MACD({p['macd_fast']},{p['macd_slow']},{p['macd_signal']})")
        print(f"   SL:{p['stop_loss_pct']}% TP:{p['take_profit_pct']}%")
        print(f"   Win Rate: {r['win_rate']:.0f}% | Return: {r['total_return']:+.1f}% | Trades: {r['num_trades']} | PF: {r['profit_factor']} | Max DD: {r['max_drawdown']:.1f}%")
    
    # Composite score ranking
    print()
    print("=" * 80)
    print("TOP 25 RISK-ADJUSTED (Composite Score)")
    print("=" * 80)
    ...
    for i, r in enumerate(by_composite, 1):
        ...
        print(f"   RSI({p['rsi_period']}) OS:{p['rsi_oversold']} OB:{p['rsi_overbought']} | MACD({p['macd_fast']},{p['macd_slow']},{p['macd_signal']})")
        print(f"   SL:{p['stop_loss_pct']}% TP:{p['take_profit_pct']}%")
        print(f"   Score: {r['composite_score']:.3f} | Return: {r['total_return']:+.1f}% | PF: {r['profit_factor']} | Win: {r['win_rate']:.0f}% | DD: {r['max_drawdown']:.1f}% | Trades: {r['num_trades']}")
    ...
    print(f"Profitable strategies: {profitable} ({profitable/len(results)*100:.1f}%)")
    print(f"Losing strategies: {len(results) - profitable} ({(len(results)-profitable)/len(results)*100:.1f}%)")
    ...
    for symbol in SYMBOLS:
        ...
        if sym_results:
            ...
            print(f"  {symbol}: {len(sym_results)} strategies | Avg: {avg_ret:+.1f}% | Best: {best_ret:+.1f}%")
    ...
    for tf in TIMEFRAMES:
        ...
        if tf_results:
            ...
            print(f"  {tf}: {len(tf_results)} strategies | Avg: {avg_ret:+.1f}% | Best: {best_ret:+.1f}%")
    ...
    for strat in STRATEGY_PARAMS['strategy_type']:
        ...
        if strat_results:
            ...
            print(f"  {strat}: {len(strat_results)} strategies | Avg: {avg_ret:+.1f}% | Best: {best_ret:+.1f}% | {prof_pct:.0f}% profitable")
